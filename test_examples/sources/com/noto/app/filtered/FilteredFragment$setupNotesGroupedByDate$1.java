package com.noto.app.filtered;

import android.content.Context;
import android.view.View;
import androidx.datastore.preferences.PreferencesProto$Value;
import com.airbnb.epoxy.m;
import com.noto.R;
import com.noto.app.domain.model.Font;
import com.noto.app.filtered.FilteredFragment;
import com.noto.app.filtered.FilteredViewModel;
import com.noto.app.folder.r;
import f7.q;
import java.util.ArrayList;
import java.util.Collection;
import java.util.Iterator;
import java.util.LinkedHashMap;
import java.util.List;
import java.util.Map;
import kotlin.Metadata;
import kotlin.Pair;
import kotlin.collections.c;
import kotlin.jvm.internal.Lambda;
import m7.n;
import na.d;
import o6.i;
import t6.a;
import u7.l;
import v7.g;
import w6.e;
import w6.f;
import x6.h0;

/* JADX INFO: Access modifiers changed from: package-private */
@Metadata(d1 = {"\u0000\f\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0002\b\u0003\u0010\u0004\u001a\u00020\u0001*\u00020\u0000H\n¢\u0006\u0004\b\u0002\u0010\u0003"}, d2 = {"Lcom/airbnb/epoxy/m;", "Lm7/n;", "invoke", "(Lcom/airbnb/epoxy/m;)V", "<anonymous>"}, k = PreferencesProto$Value.INTEGER_FIELD_NUMBER, mv = {1, 8, 0})
/* loaded from: classes.dex */
public final class FilteredFragment$setupNotesGroupedByDate$1 extends Lambda implements l<m, n> {

    /* renamed from: j  reason: collision with root package name */
    public final /* synthetic */ FilteredFragment f8241j;

    /* renamed from: k  reason: collision with root package name */
    public final /* synthetic */ Map<d, List<Pair<a, h0>>> f8242k;

    /* renamed from: l  reason: collision with root package name */
    public final /* synthetic */ Map<d, Boolean> f8243l;

    /* renamed from: m  reason: collision with root package name */
    public final /* synthetic */ Font f8244m;
    public final /* synthetic */ String n;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    /* JADX WARN: Multi-variable type inference failed */
    public FilteredFragment$setupNotesGroupedByDate$1(FilteredFragment filteredFragment, Map<d, ? extends List<Pair<a, h0>>> map, Map<d, Boolean> map2, Font font, String str) {
        super(1);
        this.f8241j = filteredFragment;
        this.f8242k = map;
        this.f8243l = map2;
        this.f8244m = font;
        this.n = str;
    }

    @Override // u7.l
    public final n U(m mVar) {
        boolean z10;
        boolean z11;
        m mVar2 = mVar;
        g.f(mVar2, "$this$withModels");
        final FilteredFragment filteredFragment = this.f8241j;
        Context j2 = filteredFragment.j();
        if (j2 != null) {
            Map<d, List<Pair<a, h0>>> map = this.f8242k;
            Collection<List<Pair<a, h0>>> values = map.values();
            if (!(values instanceof Collection) || !values.isEmpty()) {
                Iterator<T> it = values.iterator();
                while (it.hasNext()) {
                    if (!((List) it.next()).isEmpty()) {
                        z10 = false;
                        break;
                    }
                }
            }
            z10 = true;
            if (z10) {
                i iVar = new i();
                iVar.E();
                iVar.F(q.f(j2, R.string.no_notes_found, new Object[0]));
                mVar2.add(iVar);
            } else {
                for (Map.Entry<d, List<Pair<a, h0>>> entry : map.entrySet()) {
                    final d key = entry.getKey();
                    List<Pair<a, h0>> value = entry.getValue();
                    Boolean bool = this.f8243l.get(key);
                    if (bool != null) {
                        z11 = bool.booleanValue();
                    } else {
                        z11 = true;
                    }
                    ArrayList arrayList = new ArrayList(n7.l.Z1(value, 10));
                    Iterator<T> it2 = value.iterator();
                    while (it2.hasNext()) {
                        arrayList.add(Long.valueOf(((h0) ((Pair) it2.next()).f12963j).f18683a.f17621a));
                    }
                    long[] M2 = c.M2(arrayList);
                    o6.g gVar = new o6.g();
                    gVar.m(Integer.valueOf(key.a()));
                    gVar.H(f7.g.b(key));
                    gVar.p();
                    gVar.f16366l = z11;
                    View.OnClickListener onClickListener = new View.OnClickListener() { // from class: w6.c
                        @Override // android.view.View.OnClickListener
                        public final void onClick(View view) {
                            FilteredFragment filteredFragment2 = FilteredFragment.this;
                            v7.g.f(filteredFragment2, "this$0");
                            na.d dVar = key;
                            v7.g.f(dVar, "$date");
                            int i10 = FilteredFragment.f8233f0;
                            FilteredViewModel Z = filteredFragment2.Z();
                            Z.getClass();
                            Map map2 = (Map) Z.d().getValue();
                            LinkedHashMap linkedHashMap = new LinkedHashMap(m0.b.O0(map2.size()));
                            for (Map.Entry entry2 : map2.entrySet()) {
                                Object key2 = entry2.getKey();
                                boolean a10 = v7.g.a(entry2.getKey(), dVar);
                                boolean booleanValue = ((Boolean) entry2.getValue()).booleanValue();
                                if (a10) {
                                    if (!booleanValue) {
                                        booleanValue = true;
                                    } else {
                                        booleanValue = false;
                                    }
                                }
                                linkedHashMap.put(key2, Boolean.valueOf(booleanValue));
                            }
                            Z.f8297m.setValue(linkedHashMap);
                        }
                    };
                    gVar.p();
                    gVar.n = onClickListener;
                    mVar2.add(gVar);
                    if (z11) {
                        Iterator<T> it3 = value.iterator();
                        while (it3.hasNext()) {
                            Pair pair = (Pair) it3.next();
                            r rVar = new r();
                            rVar.Q(((h0) pair.f12963j).f18683a.f17621a);
                            rVar.W((h0) pair.f12963j);
                            rVar.P(this.f8244m);
                            a aVar = (a) pair.f12962i;
                            rVar.O(aVar.f17595e);
                            rVar.c0(this.n);
                            rVar.b0(aVar.f17598h);
                            rVar.V(aVar.f17601k);
                            rVar.U();
                            rVar.R(false);
                            rVar.X(new w6.d(filteredFragment, pair, M2, 0));
                            rVar.Z(new e(filteredFragment, pair, M2, 0));
                            rVar.Y(new f(0));
                            mVar2.add(rVar);
                        }
                    }
                }
            }
        }
        return n.f16010a;
    }
}
